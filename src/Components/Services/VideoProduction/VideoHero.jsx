      ...
      <div className="relative w-full flex items-center justify-center overflow-hidden" style={{ height: '420px', marginTop: '90px' }}>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-4">
          ...
          <div className="flex items-center gap-3 px-6 py-3 rounded-full mt-1" style={{ background: '#ffffff', boxShadow: '0 4px 20px rgba(80,0,180,0.12)' }}>
            ...
            <span className="text-[16px] font-semibold" style={{ color: '#1e2f57' }}>Video Production</span>
          </div>
        </div>
        ...
        <style>{`@keyframes floatBlob { 0%, 100% { transform: scale(1) translate(0, 0); } 50% { transform: scale(1.08) translate(10px, -10px); } }`}</style>
      </div>
      ...
      <section className="w-full bg-white pt-16 pb-20 px-4 md:px-8" style={{ fontFamily: "'Inter', sans-serif" }}>
        <div className="max-w-[1200px] mx-auto">
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="italic font-semibold uppercase tracking-widest" style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '13px', color: 'var(--accent-orange, #e08326)' }}>Video Production</span>
          </div>

          <ScrollRevealHeading className="mb-10" words={[
            { text: "Make" }, { text: "Your" }, { text: "Brand" }, { text: "Impossible" },
            { text: "to", italic: true }, { text: "Scroll" }, { text: "Past" }
          ]} />
          ...
          <div className="flex flex-col md:flex-row gap-8 lg:gap-20 mb-12">
            <div className="flex-1">
              <p className="leading-relaxed font-medium" style={{ fontSize: 'clamp(1rem, 2vw, 1.25rem)', color: '#334155' }}>
                People don't wait around for brands to explain themselves. A strong video has to earn attention quickly, communicate something worth remembering, and fit the platform where it will be watched.
              </p>
            </div>
            <div className="flex-1">
              <p className="leading-relaxed mb-4" style={{ fontSize: 'clamp(0.95rem, 1.6vw, 1.1rem)', color: '#475569' }}>
                ADSSERV creates brand, social, campaign, and short-form video content designed around the audience, the message, and the job the video needs to do.
              </p>
              <p className="leading-relaxed mb-10" style={{ fontSize: 'clamp(0.95rem, 1.6vw, 1.1rem)', color: '#475569' }}>
                From the first idea to the final edit, we build content made to be watched — not just uploaded.
              </p>
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <ButtonWithIcon>Start a Video Project</ButtonWithIcon>
                <button className="group w-full sm:w-auto flex items-center justify-center gap-2 px-7 py-4 font-bold rounded-full border-2 transition-all duration-300" style={{ color: 'var(--brand-navy, #1e2f57)', borderColor: 'rgba(30,47,87,0.20)', fontSize: '15px', background: 'transparent' }}>
